...
#     output$ggplot <- renderPlot({ 
    ggplotInput <- reactive({ 
        ...
        mask <-  with(dat, -log10(as.numeric(dat$P.Value))>input$hl & abs(dat$logFC)>input$vl)
        cols <- ifelse(mask, "firebrick1", "grey")
        p <- ggplot(dat2, aes(x, y, label= ID)) + geom_point(color = cols) +
            geom_vline(xintercept = input$vl, color = "dodgerblue", linetype="longdash") + #add vertical line
            geom_vline(xintercept = -input$vl, color = "dodgerblue", linetype="longdash") + #add vertical line
            geom_hline(yintercept = input$hl, color = "deeppink", linetype="longdash") +  #add vertical line
