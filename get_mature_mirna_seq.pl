...
sub get_mature_seq {
  ...
  my $dbh = DBI->connect('dbi:mysql:mirbase21','mirbase','mirbase');
  my @result;
  while(<$fh1>) {
    chomp;
    next if $_ eq '';
...
where (a.mature_name = '$mirna_id' or i.mirna_id = '$mirna_id')
...
where (a.previous_mature_id regexp '$mirna_id;|;$mirna_id|^${mirna_id}\$'
  or i.previous_mirna_id regexp '$mirna_id;|;$mirna_id|^${mirna_id}\$')
and a.auto_mature = p.auto_mature
and p.auto_mirna = i.auto_mirna";
