      ...
      foreach my $i ( @exon_order ) { ### retriving |===[%]           |
        ...
        if ($strand eq '+') {
          if (between($cdsstart+1, $exs[$i]+1, $exe[$i])
                and between($cdsend, $exs[$i]+1, $exe[$i]) ) {
            # cds in one exon
            my $len1 = $cdsend - $cdsstart;
            $len0 = $cdsstart - $exs[$i];
            $slen = $exe[$i] - $cdsend;
            $shift = $cdsstart - $exs[$i];
            $aa = coden($exon_seq, $shift,$slen);
          }
          elsif (between($cdsstart+1, $exs[$i]+1, $exe[$i]) ) {
          ...
          elsif (between($cdsend, $exs[$i]+1, $exe[$i]) ) { # start exon
            my $len1 = $cdsend - $exs[$i];
            $len0 = $exe[$i] - $cdsend;
            $slen = $len1 % 3;
            if ($slen != 0) {
