...
-resource:hapmap,known=false,training=true,truth=true,prior=15.0 ${resource_dir}/hapmap_3.3.hg38.vcf.gz \
-resource:omni,known=false,training=true,truth=true,prior=12.0 ${resource_dir}/1000G_omni2.5.hg38.vcf.gz \
-resource:1000G,known=false,training=true,truth=false,prior=10.0 ${resource_dir}/1000G_phase1.snps.high_confidence.hg38.vcf.gz \
...
# when there are only a few variant, VQSR methods could not be applied, use onle hard filter
ln -s ${out_dir}/${joint_name}/VCF/${joint_name}_RAW.vcf ${out_dir}/${joint_name}/VQSR/${joint_name}_FINAL_VQSR.vcf
